...
`tools/list` at `https://gateway.pipeworx.io/emoji-oracle/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/emoji_oracle_consult \
  -H 'Content-Type: application/json' \
  -d '{"question":"Will my next project be successful?"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/emoji_oracle_consult`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "emoji-oracle": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-emoji-oracle"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-emoji-oracle
```
